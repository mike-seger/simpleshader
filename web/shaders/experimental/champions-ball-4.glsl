...
const vec4  STAR_FILL       = vec4(0.3059, 0.4588, 1.0, 25.0);     // star interior (rgb + intensity)
const vec4  STAR_EDGE_COLOR = vec4(0.05, 0.65, 1.0, 20.0);   // star edge glow (rgb + intensity)
const vec4  SPHERE_COLOR    = vec4(0.005, 0.012, 0.035, 11.0); // sphere base (rgb + intensity)
const float SPHERE_OPACITY  = 0.38;  // 0.0 = fully transparent, 1.0 = fully opaque
const float STAR_OPACITY    = 0.2;  // 0.0 = fully transparent, 1.0 = fully opaque
