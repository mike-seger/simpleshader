...
const float SPHERE_SIZE         = 2.5;   // @range(0.5, 2.5, 0.05)
const vec3  LIGHT_DIR           = vec3(1.5, -0.1, -2.0);
const float SPIN_SPEED          = 3.4;   // @range(0.0, 20.0, 0.1)
...
const vec3  CAM_POS             = vec3(0.0, 0.576, -7.8747);
const float CAM_FOCAL           = 0.6;   // focal length (zoom) // @range(0.5, 4.0, 0.05)
const vec3  SPHERE_CENTER       = vec3(0.0, 1.7949, -0.837);
// ── Ground fog ──
const float GROUND_DEPTH        = 0.31;   // fraction of ball submerged // @range(0.0, 0.5, 0.01)
const float FOG_HEIGHT          = -2.95;   // fog layer thickness above ground // @range(-5.0, 5.0, 0.05)
const float FOG_DENSITY         = 2.1;   // fog opacity // @range(0.1, 10.0, 0.1)
const float FOG_FALLOFF         = 8.0;   // vertical fog falloff rate // @range(0.5, 10.0, 0.5)
