...
vec2 mainSound(int samp, float time) {
  ...
  float beatFrames = iSampleRate * B2T;
  float sampF = float(samp);
  vec4 time4 = vec4(
    mod(sampF, beatFrames),
    mod(sampF, beatFrames * 4.0),
    mod(sampF, beatFrames * 64.0),
    mod(sampF, beatFrames * 65536.0)
  ) / iSampleRate;

  return mainAudio(time4);
}
